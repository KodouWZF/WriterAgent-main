...
async def DocumentSearch(
    ...
    print(f"工具上下文: {tool_context}")

    try:
        start_time = time.time()
        print(f"开始调用search_api，关键词: {keyword}")
        results = search_api(keyword, limit=10)
        api_call_time = time.time() - start_time
        print(f"search_api调用完成，耗时: {api_call_time:.2f}秒")
        
        if not results or not results.get("articles"):
            print(f"警告: 没有搜索到'{keyword}'相关的文章")
            print("=" * 50)
            return f"没有搜索到{keyword}相关的文章"
        
        print(f"成功获取到搜索结果，文章数量: {len(results.get('articles', []))}")
        ...
        for i, article_dict in enumerate(articles):
            ...
            print(f"  处理文章 {i+1}/{len(articles)}: {article_dict.get('title', 'Unknown')}")
            ...
            if file_id not in references:
                ...
                print(f"    新增文章到references: {article_title}")
        ...
        for i, article_dict in enumerate(articles):
            ...
            print(f"  格式化文章 {i+1}/{len(articles)}: {reference_info.get('title', 'Unknown')}")
            ...
            article_text = f"{idx_val}. 标题：{title}\n时间: {publish_time}\n内容: {snippet_content}\n作者: {author}.\n期刊: {journal}.\nDOI: {docdoi}\n\n"
            articles_content.append(article_text)
            print(f"    文章格式化完成，长度: {len(article_text)} 字符")

        # 重新设置 tool_context 中的 references
        tool_context.state["references"] = references
        print(f"更新references完成，当前references总数: {len(references)}")
        
        print("DocumentSearch工具执行完成")
        print("=" * 50)
        return articles_content
        
    except Exception as e:
        print(f"DocumentSearch执行过程中发生错误: {str(e)}")
