...
class FastCheckerAgent(BaseAgent):
    ...
    async def _run_async_impl(self, ctx: InvocationContext) -> AsyncGenerator[Event, None]:
        ...
        # # 当current_part_index为1时，修改####和###后面的数字为1
        # if current_part_index == 1:
        #     import re
        #     # 将####后面且在.前面的数字改为1
        #     last_draft = re.sub(r'(####\s*)\d+(?=\s*\.)', r'\g<1>1', last_draft)
        #     # 将###后面的数字改为1
        #     last_draft = re.sub(r'(###\s*)\d+', r'\g<1>1', last_draft)
        #     # 更新修改后的内容到session state
        #     ctx.session.state["last_draft"] = last_draft
        ...
        try:
            if current_part_index == 1:
                print("第一部分内容为：", last_draft)

            # 确定当前处理的内容块类型
            # 如果current_part_index为0，则表示正在处理摘要部分(ABSTRACT)
            # 否则表示正在处理正文部分(SECTION)
            block_type = "ABSTRACT" if current_part_index == 0 else "SECTION"
            
            # 执行基于正则表达式的格式检查
            # 调用RegexChecker的check_content方法对内容进行验证
            # 传入的参数包括:
            # - content: 待检查的文本内容(last_draft)
            # - title: 文章标题
            # - struct: 当前块的结构定义(last_struct)
            # - language: 内容语言
            # - block_type: 块类型(ABSTRACT或SECTION)
