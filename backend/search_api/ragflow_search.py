...
此模块提供基于Ragflow的本地知识库检索功能，
用于在使用外部搜索引擎（如智谱）之前先检索本地向量库。
"""
...
# ===================== 配置 =====================
# 相似度阈值，低于此值的结果将被过滤掉
SIMILARITY_THRESHOLD = float(os.environ.get("RAGFLOW_SIMILARITY_THRESHOLD", "0.3"))
...
class RagflowSearch:
    ...
    def __init__(self):
        ...
        """
        try:
            self.client = RagflowSearchClient()
            self.initialized = True
        except Exception as e:
            print(f"[ragflow_search] 初始化失败: {e}")
            self.initialized = False
    
    def _make_snippet(self, text: str, length: int = 160) -> str:
        """
        从文本中创建摘要片段
    ...
    def _compute_file_id(self, title: str, publish_time: Optional[str] = None) -> str:
        """
        根据标题和发布时间生成稳定的file_id
    ...
    def search_local_knowledge_base(self, keyword: str, limit: int = 10) -> List[Dict[str, Any]]:
        """
        搜索本地知识库
        ...
        Returns:
            搜索结果列表，每个元素包含id, file_id, title, snippet, score, publish_time, url等字段
        """
        # 检查客户端是否初始化成功
        if not self.initialized:
            print("[ragflow_search] RAGflow客户端未初始化，跳过本地检索")
            return []
        
        print(f"[ragflow_search] 开始本地知识库检索: {keyword}")
        
        # 调用RAGflow客户端进行搜索
        success, results = self.client.search_sync(
            query=keyword,
            limit=limit,
            similarity_threshold=SIMILARITY_THRESHOLD
        )
        ...
        if not success:
            print("[ragflow_search] 本地知识库检索失败")
            return []
        
        print(f"[ragflow_search] 本地知识库检索成功，找到 {len(results)} 个结果")
        ...
        for item in results:
            ...
            # 生成file_id
            file_id = self._compute_file_id(title)
            ...
            result_item = {
                ...
                "score": score,
                "publish_time": None,  # RAGflow结果中不包含发布时间
                "url": f"ragflow://{document_id}",  # 使用特殊协议标识来自RAGflow的结果
                "content": content
            }
...
# 创建全局实例
ragflow_search_instance = RagflowSearch()

@cache_decorator
def ragflow_search_web(keyword: str, number: int = 10) -> Tuple[bool, List[Dict[str, Any]]]:
    """
    通过RAGflow搜索本地知识库的包装函数
    ...
    Returns:
        (状态, 结果列表) 的元组
    """
    print(f"[ragflow_search_web] 开始搜索: {keyword}")
    
    try:
        results = ragflow_search_instance.search_local_knowledge_base(keyword, number)
        if results:
            print(f"[ragflow_search_web] 搜索成功，返回 {len(results)} 个结果")
            return True, results
        else:
            print(f"[ragflow_search_web] 未找到相关结果")
            return False, []
    except Exception as e:
        print(f"[ragflow_search_web] 搜索过程中发生错误: {e}")
        return False, []
...
if __name__ == "__main__":
    # 测试RAGflow搜索功能
    test_keyword = "煤化工业研究"
    success, results = ragflow_search_web(test_keyword, 5)
    
    if success:
        print(f"\n搜索关键词 '{test_keyword}' 的结果:")
        ...
        print(f"搜索关键词 '{test_keyword}' 失败或无结果")