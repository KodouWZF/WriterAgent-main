...
# ======== 导入RAGflow搜索模块 ========
try:
    from ragflow_search import ragflow_search_web
    RAGFLOW_AVAILABLE = True
except ImportError:
    RAGFLOW_AVAILABLE = False
    print("[search_api] RAGflow搜索模块不可用")
...
def _search_articles_sync(keyword: str, limit: int, search_engine: str="default") -> List[ArticleHit]:
    ...
    """
    # 如果启用了RAGflow，先尝试本地检索
    local_results = []
    if RAGFLOW_AVAILABLE:
        print(f"[search_api] 尝试使用RAGflow进行本地检索: {keyword}")
        try:
            success, results = ragflow_search_web(keyword=keyword, number=limit)
            if success and results:
                print(f"[search_api] RAGflow本地检索成功，找到 {len(results)} 个结果")
                local_results = results
            else:
                print(f"[search_api] RAGflow本地检索未找到结果")
        except Exception as e:
            print(f"[search_api] RAGflow本地检索出错: {e}")
    ...
    if local_results:
        ...
        for item in local_results:
            ...
            hits.append(hit)
        # try :
        #     with open("debug_local_results.json", "w", encoding="utf-8") as f:
        #         json.dump(local_results, f, ensure_ascii=False, indent=2)
        # except Exception as e:
        #     print(f"[search_api] 写入 debug_local_results.json 失败: {e}")
