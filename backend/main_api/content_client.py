    ...
    async def generate(self, user_question: str, language="chinese", user_id="") -> None:
        ...
        async with httpx.AsyncClient(timeout=600.0) as httpx_client:
            ...
            async for chunk in stream_response:
                chunk_index += 1
                self.logger.info(f"输出的第 {chunk_index} 个chunk内容: {chunk}")
                ...
                if result.get("kind") == "status-update":
                    ...
                    if chunk_status_state == "submitted":
                        ...
                        # 仍然需要通知前端任务已提交
                        yield {"type": "status", "status": "submitted", "author": "system"}
                        continue
                    elif chunk_status_state == "working":
                        self.logger.info("任务处理中")
                        # 通知前端任务正在进行中
                        yield {"type": "status", "status": "working", "author": "system"}
                    ...
                    if references:
                        ...
                        self.logger.info(f"收到参考文献元数据: {len(references)} 条")
                        yield {"type": "metadata", "metadata": metadata, "author": author}
                    if parts:
                        for part in parts:
                            part_kind = part["kind"]
                            self.logger.info(f"status, part类型: {part_kind}, 内容长度: {len(part.get('text', ''))}")
                            print(f"status, {part}")
                            if part_kind == "data":
                                print(f"收到的是data内容:")
                                self.logger.info(f"收到data内容: {part.get('data', '')}")
                                yield {"type": "data", "data": part["data"], "author":author}
                            else:
                                # text文本
                                text_content = part["text"]
                                text_preview = text_content[:100].replace('\n', '\\n')
                                self.logger.info(f"收到text内容，长度: {len(text_content)}, 预览: {text_preview}")
                                yield {"type": "text", "text": text_content, "author": author}
                    ...
                    if parts:
                        for part in parts:
                            self.logger.info(f"artifact, {part}")
                    ...
                    chunk_status = result["status"]
                    self.logger.info(f"任务的状态是: {chunk_status}")
                else:
                    self.logger.warning(f"未识别的chunk类型: {result.get('kind')}")
            self.logger.info(f"=== Agent正常处理完成，对话结束。总共处理了 {chunk_index} 个chunks ===")
            yield {"type": "final", "text": "对话结束", "author": "system"}

if __name__ == '__main__':
    async def main():
        session_id = time.strftime("%Y%m%d%H%M%S", time.localtime())
        wrapper = A2AContentClientWrapper(session_id=session_id, agent_url="http://localhost:10051")
